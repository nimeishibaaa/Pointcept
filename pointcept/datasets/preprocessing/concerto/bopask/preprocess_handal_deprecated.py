...
def process_scene(scene_id, images_dir, depth_dir, masks_dir, camera_json_path, output_root, rgb_gap=5, voxel_size=0.005):
    ...
    for rgb_path in tqdm(rgb_files, desc="Unprojecting frames"):
        ...
        depth_in_m = (depth_img.astype(np.float32) * depth_scale) / 1000.0 # Convert mm to meters
        ...
        # VERY IMPORTANT: Re-read K exactly as in the test script to ensure identical unprojection
        fx, fy, cx, cy = K[0,0], K[1,1], K[0,2], K[1,2]
        x_all = (u_all.astype(np.float32) - cx) * z_all / fx
        y_all = (v_all.astype(np.float32) - cy) * z_all / fy
        ...
        # Reconstruct the final valid arrays (Using strict boolean indexing without any offset shifts)
        z = z_all[in_box]
        x = x_all[in_box]
        y = y_all[in_box]
        ...
        # Transform to Original World Space using Strict Math exactly as in test script
        if use_bopask_twc:
            pts_cam_homo = np.hstack((pts_cam, np.ones((pts_cam.shape[0], 1))))
            pts_world_orig = (T_c2w @ pts_cam_homo.T).T[:, :3]
        else:
            # STRICT MATH FIX: Must match test_bop_raw_strict.py EXACTLY
        ...
        # Only apply rotation to the points AFTER they have been successfully unprojected
        # into the valid BOP global coordinate space.
        ...
        if use_bopask_twc:
            ...
            # When using BOP raw extrinsics, the original pose is P_cam = R_w2c * P_world + t_w2c
            # The inverse (T_c2w) would normally be [R_w2c^T, -R_w2c^T * t_w2c]
            # After scene alignment: P_aligned = R_align * (P_world - C)
            # => P_world = R_align^T * P_aligned + C
            # Substitute into camera equation: P_cam = R_w2c * (R_align^T * P_aligned + C) + t_w2c
            # P_cam = (R_w2c * R_align^T) * P_aligned + (R_w2c * C + t_w2c)
            # Therefore, the new Camera-to-World matrix (T_c2w_aligned) which maps P_cam to P_aligned is the inverse of this:
            # T_c2w_aligned_R = R_align * R_w2c^T
            # T_c2w_aligned_t = -R_align * R_w2c^T * (R_w2c * C + t_w2c) = -R_align * C - R_align * R_w2c^T * t_w2c
            
            T_c2w_aligned[:3, :3] = R_align @ np.linalg.inv(R_w2c)
            T_c2w_aligned[:3, 3] = R_align @ (-np.linalg.inv(R_w2c) @ t_w2c) - R_align @ scene_centroid
